...
DEFAULT_CERTS_DIR=/data/local/tmp/cert
echo "[i] copy default certificates to the old location and set the ownership and permissions"
cp -f ${DEFAULT_CERTS_DIR}/* ${MODDIR}/system/etc/security/cacerts/
chown -R 0:0 ${MODDIR}/system/etc/security/cacerts
chmod 644 ${MODDIR}/system/etc/security/cacerts/*
set_context /system/etc/security/cacerts ${MODDIR}/system/etc/security/cacerts
...
if [ -d /apex/com.android.conscrypt/cacerts ]; then
    echo "[i] copy the cert and set the ownership and permissions"
    cp -f ${DEFAULT_CERTS_DIR}/* /data/local/tmp/adg-ca-copy/
else
    echo "[i] APEX container not found, this must be Android < 14"
fi
